...
app.prepare().then(() => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("openPit", () => {
      ...
      io.emit("glitchLog", {
        type: "rumble",
        message: `THE PIT IS OPEN! Enter with your bet. 1 = Duel vs House (2x). 2-3 = Rumble. Max ${MAX_PIT_PLAYERS}.`,
      });
      io.emit("rumbleState", rumbleState);
    });
    ...
    socket.on("betOnWarrior", (data) => {
      ...
      socket.emit("balanceUpdate", { balance: char.balance });
      io.emit("glitchLog", { type: "rumble", message: `${char.name} bet ${amount} PITS on ${warrior.name} (${multiplier}x). Potential win: ${Math.floor(amount * multiplier)} PITS.` });
      io.emit("rumbleState", rumbleState);
    });
    ...
    socket.on("runRumble", () => {
      ...
      if (isDuel) {
        ...
        io.emit("glitchLog", { type: "rumble", message: `The PIT erupts! Seed: ${rumbleState.seedId}. 1 vs House. Pot: ${rumbleState.prizePool} PITS. Defeat the Sentinel to double your data.` });
      } else {
        io.emit("glitchLog", { type: "rumble", message: "[SYSTEM]: Rumble Detected. Winner Takes All." });
        io.emit("glitchLog", { type: "rumble", message: `The PIT erupts! Seed: ${rumbleState.seedId}. ${rumbleState.participants.length} fighters. Pot: ${rumbleState.prizePool} PITS. Only one can remain.` });
      }

      let participantsForRumble = rumbleState.participants.map((p) => ({ ...p, isAlive: true }));
      if (isDuel) {
        participantsForRumble = [...participantsForRumble, { id: SYSTEM_SENTINEL_ID, name: SYSTEM_SENTINEL_NAME, isAlive: true, loreClass: "The House" }];
      } else {
      ...
      if (isDuel) rumbleState.participants.push({ id: SYSTEM_SENTINEL_ID, name: SYSTEM_SENTINEL_NAME, isAlive: true, loreClass: "The House" });
      rumbleState.events = [];
      io.emit("rumbleState", rumbleState);
        ...
        if (idx >= events.length) {
          ...
          if (isDuel) {
            ...
            if (humanWinner) {
              ...
              leaderboard.unshift({ name: result.winner.name, amount: duelPayout, seedId: rumbleState.seedId, date: new Date().toISOString() });
            ...
            if (result.winner) {
              ...
              const winnerBets = Object.entries(rumbleState.spectatorBets || {}).filter(([_, b]) => b.warriorId === result.winner.id);
              ...
              leaderboard.unshift({ name: result.winner.name, amount: grandPayout, seedId: rumbleState.seedId, date: new Date().toISOString() });
              if (leaderboard.length > 10) leaderboard.pop();
              emitLeaderboard();
              io.emit("glitchLog", { type: "winner", message: `🏆 ${result.winner.name} WINS! Took ${grandPayout} PITS (after ${HOUSE_FEE_PERCENT}% fee). Seed: ${rumbleState.seedId}` });
