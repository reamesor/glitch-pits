/**
 * Glitch Pits - Enter the pit with your bet. Winner takes all.
 * Users don't control characters - automated rumble with lore.
 */
...
io.on("connection", (socket) => {
  ...
  socket.on("runRumble", () => {
    ...
    const stats = Object.fromEntries(
      rumbleState.participants.map((p) => [p.id, { attack: p.attack, defense: p.defense, luck: p.luck }])
    );
    ...
    if (result.winner) {
      ...
      io.emit("glitchLog", {
        type: "winner",
        message: `🏆 ${result.winner.name} WINS! Took all ${rumbleState.prizePool} PITS.`,
      });
